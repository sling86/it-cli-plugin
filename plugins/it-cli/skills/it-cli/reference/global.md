# Global commands (no provider)

Commands invoked as `its <command>`, without a provider — cross-provider lookups, configuration, and session management. Every one accepts `--help`.
...
## `its auth`
Microsoft delegated sign-in — profiles, provider mapping, token diagnostics. **changes state** · **interactive** — prompts, needs a TTY.
...
## `its log`
Append a timestamped entry to today's vault daily note. **changes state**.
...
## `its setup`
Interactive setup wizard — configure the CLI or one provider. **changes state** · **interactive** — prompts, needs a TTY.
...
## `its tui`
Interactive browser — provider, resource, action, no flags to memorise. **interactive** — prompts, needs a TTY.
